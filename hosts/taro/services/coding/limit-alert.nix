# Poll aggregate OOM/PID counters; MemoryHigh throttling is intentionally ignored.
{
  ...
  slice = "user-${toString config.users.users.coding.uid}.slice";
  cg = "/sys/fs/cgroup/user.slice/${slice}";

  script = pkgs.writeShellScript "coding-limit-alert" ''
    set -u
    ...
    # Counters reset when the slice is recreated.
    ...
    {
      printf 'To: root\n'
      printf 'Subject: [${hostName}] coding hit a resource limit\n'
  ...
  '';
in {
  systemd.timers.coding-limit-alert = {
  ...
  systemd.services.coding-limit-alert = {
    description = "Mail on aggregate coding cgroup limit counters";
    after = ["network-online.target"];
    wants = ["network-online.target"];
    serviceConfig = {
      Type = "oneshot";
      ExecStart = script;
      StateDirectory = "coding-limit-alert";
    };
  };
}
